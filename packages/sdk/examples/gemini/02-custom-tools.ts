/**
 * 02 — Gemini agent with custom tools.
 *
 * Adds two `CustomToolDef`s — a calculator and a (mock) weather tool —
 * alongside the platform tools. Gemini's function-calling loop sees them
 * as function declarations; the adapter dispatches the call to your
 * handler and feeds the result back to the model.
 *
...
export function createToolsAgent(
  ...
  return Agent.create({
    adapter,
    config: {
      agentId: overrides?.agentId ?? "gemini-tools-agent",
...
if (isDirectExecution(import.meta.url)) {
  ...
  const config = loadAgentConfig("gemini_tools_agent");
