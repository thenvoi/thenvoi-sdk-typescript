/**
 * 02 — OpenAI agent with custom tools.
 *
 * Adds two `CustomToolDef`s — a calculator and a (mock) weather tool —
 * alongside the platform tools. The OpenAI tool-calling loop sees them as
 * function definitions; the adapter dispatches the call to your handler
...
export function createToolsAgent(
  ...
  return Agent.create({
    adapter,
    config: {
      agentId: overrides?.agentId ?? "openai-tools-agent",
...
if (isDirectExecution(import.meta.url)) {
  ...
  const config = loadAgentConfig("openai_tools_agent");
